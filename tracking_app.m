...
%% ── Control Panel ────────────────────────────────────────────────────────
% cp is the SCROLL VIEWPORT - a fixed-size visible panel.
% cpInner is a TALL inner panel that actually holds every control. Because
% cpInner is taller than cp, MATLAB shows a scroll bar on cp and the user
% can scroll cpInner up and down inside it. This is the canonical MATLAB
% pattern for scrollable controls; setting Scrollable='on' directly with
% children at negative y values does NOT reliably engage scrolling.
cpW = round(272*sx);
cpH = round(944*sy);
cp = uipanel(fig,'Title','Controls', ...
    'Position',[MARGIN_OUTER MARGIN_OUTER cpW cpH], ...
...
% Inner panel taller than cp; positioned at cp's bottom-left so its TOP
% extends above cp's visible area. Combined with `scroll(cp,'top')`
% below, this gives the natural UX: the top of the controls is shown by
% default, and scrolling DOWN reveals the preprocessing block at the
% bottom. Positioning cpInner below cp (negative y) is *not* reliably
% scrolled to by MATLAB's uipanel Scrollable in practice.
innerH = round(1800*sy);    % tall enough to fit every control with margin
cpInner = uipanel(cp, ...
    'BorderType','none', ...
    'BackgroundColor',[0.16 0.16 0.18], ...
    'Position',[0, 0, cpW, innerH]);

% Layout starts at top of cpInner (with a 64px top margin). All control
% y-coords below use cpInner's local frame; positive y means visible at
% the top, decreasing y means farther down (potentially below the viewport
% and only reachable by scrolling).
y = innerH - round(64*sy);

btnLoad = uibutton(cpInner,'push', ...
    'Text','  Load Image', ...
...
btnTemplate = uibutton(cpInner,'push', ...
    'Text','📂  Load Template', ...
    'Position',[PADDING_PANEL y round(244*sx) BTN_HEIGHT], ...
...
btnPause = uibutton(cpInner,'push', ...
    'Text','⏸ Pause', ...
...
lblMotionThresh = uilabel(cpInner, ...
    'Position',[PADDING_PANEL y round(244*sx) LABEL_HEIGHT], ...
    'Text','Motion Threshold: 1.5 px', ...
    'FontSize',10,'FontColor',[0.75 0.85 0.95], ...
    'Visible','off');
y = y - GAP_LABEL_TO_CONTROL;
slMotionThresh = uislider(cpInner, ...
    'Limits',[0.1 10], 'Value',1.5, ...
    'Position',[PADDING_PANEL y round(244*sx) 3], ...
    'MajorTicks',[0.1 0.5 1 2 4 6 8 10],'MinorTicks',[], ...
    'Visible','off');
y = y - round(58*sy);
...
btnReset = uibutton(cpInner,'push', ...
    'Text','↺ Reset Defaults', ...
    'Position',[PADDING_PANEL y round(244*sx) BTN_HEIGHT], ...
    'BackgroundColor',[0.40 0.42 0.50], ...
    'FontColor',[1 1 1],'FontWeight','bold','FontSize',12, ...
    'Visible','off');
y = y - GAP_SECTION;

makeLabel(cpInner,'Category',[PADDING_PANEL y round(244*sx) LABEL_HEIGHT]);
y = y - GAP_LABEL_TO_CONTROL;
ddCat = uidropdown(cpInner,'Items',{'Enhancement','Spatial Filtering', ...
...
makeLabel(cpInner,'Operation',[PADDING_PANEL y round(244*sx) LABEL_HEIGHT]);
y = y - GAP_LABEL_TO_CONTROL;
ddOp = uidropdown(cpInner,'Items',{'Brightness','Histogram Equalization'}, ...
       'Position',[PADDING_PANEL y round(244*sx) CONTROL_HEIGHT]);
y = y - GAP_SECTION;

lblParam = makeLabel(cpInner,'Parameter: 0.00',[PADDING_PANEL y round(244*sx) LABEL_HEIGHT]);
y = y - GAP_LABEL_TO_CONTROL;
slParam  = uislider(cpInner,'Limits',[-1 1],'Value',0,'Position',[PADDING_PANEL y round(244*sx) 3], ...
           'MajorTicks',[],'MinorTicks',[]);
y = y - GAP_SECTION - round(15*sy);  % Extra space before Butterworth order sliders

lblLP = makeLabel(cpInner,'Butterworth LP Order n: 2',[PADDING_PANEL y round(244*sx) LABEL_HEIGHT]);
lblLP.Visible='off';
y = y - GAP_LABEL_TO_CONTROL;
slLP = uislider(cpInner,'Limits',[1 10],'Value',2,'Position',[PADDING_PANEL y round(244*sx) 3], ...
       'MajorTicks',[],'MinorTicks',[],'Visible','off');
y = y - GAP_SECTION;

lblHP = makeLabel(cpInner,'Butterworth HP Order n: 2',[PADDING_PANEL y round(244*sx) LABEL_HEIGHT]);
lblHP.Visible='off';
y = y - GAP_LABEL_TO_CONTROL;
slHP = uislider(cpInner,'Limits',[1 10],'Value',2,'Position',[PADDING_PANEL y round(244*sx) 3], ...
       'MajorTicks',[],'MinorTicks',[],'Visible','off');
y = y - GAP_SECTION;

% ── Pre-Processing section ────────────────────────────────────────────────
lblPreprocHeader = uilabel(cpInner,'Position',[PADDING_PANEL y round(244*sx) LABEL_HEIGHT], ...
    'Text','─── Pre-Processing ───', ...
    'FontSize',10,'FontColor',[0.6 0.75 0.9],'HorizontalAlignment','center');
y = y - round(28*sy);

chkForceGray = uicheckbox(cpInner, ...
    'Text','Force Grayscale', ...
    'Position',[PADDING_PANEL y round(244*sx) round(22*sy)], ...
    'FontColor',[0.9 0.9 0.9],'FontSize',11,'Value',false);
y = y - round(28*sy);

chkHistEq = uicheckbox(cpInner, ...
    'Text','Histogram Equalization', ...
    'Position',[PADDING_PANEL y round(244*sx) round(22*sy)], ...
    'FontColor',[0.9 0.9 0.9],'FontSize',11,'Value',false);
y = y - round(32*sy);

chkBoxFilt = uicheckbox(cpInner, ...
    'Text','Box Filter', ...
    'Position',[PADDING_PANEL y round(244*sx) round(22*sy)], ...
    'FontColor',[0.9 0.9 0.9],'FontSize',11,'Value',false);
y = y - round(28*sy);

lblBoxSize = uilabel(cpInner,'Position',[PADDING_PANEL y round(244*sx) LABEL_HEIGHT], ...
    'Text','Kernel: 3x3', ...
    'FontSize',10,'FontColor',[0.75 0.85 0.75],'Visible','off');
y = y - GAP_LABEL_TO_CONTROL;
slBoxSize = uislider(cpInner,'Limits',[1 15],'Value',3, ...
    'Position',[PADDING_PANEL y round(244*sx) 3], ...
    'MajorTicks',[1 3 5 7 9 11 13 15],'MinorTicks',[],'Visible','off');
y = y - round(22*sy);

chkGauss = uicheckbox(cpInner, ...
    'Text','Gaussian Filter', ...
    'Position',[PADDING_PANEL y round(244*sx) round(22*sy)], ...
    'FontColor',[0.9 0.9 0.9],'FontSize',11,'Value',false);
y = y - round(26*sy);

lblGaussSigma = uilabel(cpInner,'Position',[PADDING_PANEL y round(244*sx) LABEL_HEIGHT], ...
    'Text','Sigma: 1.0', ...
    'FontSize',10,'FontColor',[0.75 0.85 0.75],'Visible','off');
y = y - GAP_LABEL_TO_CONTROL;
slGaussSigma = uislider(cpInner,'Limits',[0.5 5],'Value',1, ...
    'Position',[PADDING_PANEL y round(244*sx) 3], ...
    'MajorTicks',[0.5 1 2 3 4 5],'MinorTicks',[],'Visible','off');
y = y - round(22*sy);

chkSobelH = uicheckbox(cpInner, ...
    'Text','Sobel H (preprocess)', ...
    'Position',[PADDING_PANEL y round(244*sx) round(22*sy)], ...
    'FontColor',[0.9 0.9 0.9],'FontSize',11,'Value',false);
y = y - round(26*sy);

chkSobelV = uicheckbox(cpInner, ...
...
% cpInner extends ABOVE cp's visible area, so by default MATLAB shows the
% BOTTOM of cpInner (preprocessing block) and the top buttons are above
% the viewport. Programmatically scroll to the top so the user starts on
% the Load Video button and scrolls DOWN through sliders to preprocessing.
drawnow;
try
    scroll(cp, 'top');
catch
    % older MATLAB versions may not have scroll() for uipanel; ignore.
end
...
function cb_LoadTrackingVideo(fig)
    ...
    % Initialize scene-flow on the first frame: detect Harris corners
...
% Restore every user-controllable setting to its default value and
% re-initialise the tracker from the video's first frame.
    d = fig.UserData;
    if ~strcmp(d.h.ddCat.Value,'Object Tracking')
        return;
    end
    ...
    d.h.chkHistEq.Value    = false;
    d.h.chkBoxFilt.Value   = false;
    ...
    d.h.lblGaussSigma.Text = 'Sigma: 1.0';
    % Hide the box / gauss size sliders since their checkboxes are now off.
    d.h.lblBoxSize.Visible    = 'off';
    d.h.slBoxSize.Visible     = 'off';
    d.h.lblGaussSigma.Visible = 'off';
    d.h.slGaussSigma.Visible  = 'off';

    fig.UserData = d;
    relayoutPreproc(fig);

    % Re-initialise the tracker from the first frame so new BlockSize /
    % MinQuality / Pyramid Levels are actually applied.
